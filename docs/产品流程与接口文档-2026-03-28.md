...
- 当前实现暂无独立 `GET /api/v1/content/rules/summary` / `GET /api/v1/competition/landing`
- 未登录首页活动信息与规则入口目前以前端静态内容承接
...
- `GET /api/v1/auth/avatar-files/{filename}`
...
- 当前实现暂无独立封锁态查询接口，登录前封锁弹窗仍为前端占位流程
...
- `GET /api/v1/account/asset-trend?range=7d|30d`
...
```json
{
  ...
  "data": {
    ...
    "userId": "u_2f8a9c4e7b6d43c1a5e2f9b8c7d6a4e1",
    "phone": "91234567",
...
- 请求头必须带 `X-User-Id`
- `nickname` 最长 8 个字符
- `avatarId` 可为预设头像白名单（如 `a1~a6`）或上传接口返回的文件路径

### 7.1.4 上传头像

- Method：`POST`
- Path：`/api/v1/auth/avatar-upload`
- 用途：上传自定义头像文件

请求：

- Header：`X-User-Id: <real-user-id>`
- Content-Type：`multipart/form-data`
- FormData：`file=<png|jpg|webp|gif, <=2MB>`
...
```json
{
  ...
  "msg": "success",
  "data": {
    "avatarId": "/api/v1/auth/avatar-files/u_2f8a9c4e7b6d43c1a5e2f9b8c7d6a4e1_1.png",
    "avatarUrl": "/api/v1/auth/avatar-files/u_2f8a9c4e7b6d43c1a5e2f9b8c7d6a4e1_1.png",
    "avatarVersion": 1
  }
}
```
...
- Method：`GET`
- Path：`/api/v1/auth/avatar-files/{filename}`
- 用途：回显已上传头像文件

说明：

- 返回文件流，不走 `Result<T>` 包装
- `filename` 需使用上传接口返回的真实文件名

### 7.1.6 会话状态检查

- Method：`GET`
- Path：`/api/v1/auth/session`
- 用途：登录前后统一查询当前会话态与资料完成态
...
- 未携带 `X-User-Id` 时返回 `loggedIn=false`，`userId/phone/accountStatus` 为 `null`
- 该接口用于状态检查，不替代验证码登录与资料提交流程
...
- 返回个人资产汇总、头像昵称、排名、可交易次数、现金/冻结/在途资金等字段
- 返回体包含 `lang`
...
- 用途：个人页资产走势折线图
- Header：`X-User-Id: <real-user-id>`
- Query：`range=7d|30d`（可选，非法值回退 `7d`）
...
```json
{
  ...
  "updatedAt": "2026-03-31T21:00:00+08:00",
  "lang": "zh-Hant"
}
```
...
- Header：`X-User-Id: <real-user-id>`
- 说明：当前无 `tab` 查询参数，返回 `currentUserId`、`total`、`items`、`updatedAt`、`lang`

### 7.3.2 20 大港股持仓

- Method：`GET`
- Path：`/api/v1/leaderboard/top-holdings`
- 说明：当前无 `limit` 查询参数，返回 `total`、`items`、`updatedAt`、`lang`

### 7.3.3 10 大成交港股

- Method：`GET`
- Path：`/api/v1/leaderboard/top-turnover`
- 说明：当前无 `side` / `limit` 查询参数，接口一次性返回 `buy` 与 `sell` 两组榜单

### 7.3.4 参赛者 20 大蚀钱港股持仓

- Method：`GET`
- Path：`/api/v1/leaderboard/top-loser-holdings`
- 说明：返回 `total`、`items`、`updatedAt`、`lang`
...
```json
{
  ...
  "data": {
    ...
    "quantity": 100,
    "lots": 1,
    "estimatedAmount": 32340.0,
    "estimatedFee": 153.93,
    "estimatedTotalCost": 32493.93,
    ...
    "validUntil": "2026-03-28T16:00:00+08:00",
...
- Method：`GET`
- Path：`/api/v1/trade/quote/stream`
- Query：`stockCode=00700`
- 用途：先拉快照后持续订阅增量行情与买卖盘

说明：

- 返回 `text/event-stream`
- 当前仅要求 `stockCode`，不强制 `X-User-Id`

### 7.4.5 本地调试推送行情

- Method：`POST`
- Path：`/api/v1/trade/quote/debug-push`
- Query：`forceMatch=true|false`
- 用途：本地联调注入行情并触发撮合

说明：

- 若时间戳落后于最新行情，则返回 `accepted=false`、`reason=STALE_MARKET_DATA`
...
- 用途：确认弹窗点击确定后正式提交
- Header：`X-User-Id: <real-user-id>`
- Header（可选）：`X-Idempotency-Key: <unique-key>`
...
- 当前实现暂无独立 `/api/v1/content/rules` / `/api/v1/content/help`
- 更多页、规则页、帮助说明仍以前端静态内容承接
